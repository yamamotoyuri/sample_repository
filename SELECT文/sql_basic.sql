...
SELECT * FROM countries WHERE indep_year < '1990-01-01' OR population > 100000;
...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(name,"の人口は", population,"です") AS POPULASTION FROM countries;
...
SELECT region,life_expectancy,AVG(population) AS "平均人口" FROM countries GROUP BY region;
...
SELECT MIN(surface_area) AS "アジアの最小の表面積" FROM countries GROUP BY continent HAVING continent= "Asia";

-- 問24
-- アジア大陸の表面積の合計を表示してください。
SELECT SUM(surface_area) AS "アジア大陸の表面積の合計" FROM countries GROUP BY continent= "Asia";

-- 問25
-- 全ての国と言語を表示してください。一つの国に複数言語があると思いますので同じ国名を言語数だけ出力してください。
SELECT countries.name, countrylanguages.language FROM countries LEFT OUTER JOIN countrylanguages ON countries.code =countrylanguages.country_code;

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT countries.name AS '国名',cities.name AS'市区町村名',countrylanguages.language FROM countries LEFT JOIN cities ON countries.code=cities.country_code LEFT JOIN countrylanguages ON countries.code=countrylanguages.country_code;
...
SELECT celebrities.name, countries.name, countrylanguages.language FROM celebrities JOIN countries ON celebrities.country_code = countries.code JOIN countrylanguages ON celebrities.country_code = countrylanguages.country_code WHERE is_official ='T';
...
SELECT '1991' AS "誕生月", COUNT(*) AS "人数" FROM celebrities WHERE birth LIKE '1991%' UNION
SELECT '1981' AS "誕生月", COUNT(*) AS "人数" FROM celebrities WHERE birth LIKE '1981%';

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
SELECT countries.name AS "国名", AVG(celebrities.age) AS "平均年齢" FROM countries JOIN celebrities ON  countries.code=celebrities.country_code
GROUP BY countries.name ORDER BY AVG(celebrities.age)  DESC;
